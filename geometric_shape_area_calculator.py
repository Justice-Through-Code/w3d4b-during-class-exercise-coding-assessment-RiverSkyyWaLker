...
def main():
    ...
    if choice == 1:  #DO NOT REMOVE THE 'IF'
        ...
        length = input("What is the width of the rectangle?")
        ...
        base = input("What is base length of the triangle?")
    ...
    print("Accept the challenge via the GitHub Classroom link (if you're already here, you've done this part! \
          Clone the repository to your computer. Follow the instructions in geometric_shape_area_calculator.py. \
          ...
          From the main folder of the challenge, run your computer's version of `python3 -m unittest` to see if your code works as expected. \
          Perform commands for git add, git status, git commit, git push. \
          Access your GitHub Repository: Go to the GitHub website and log in to your account. \
          Locate the repository for this exercise.\
          Watch for the Red X (or yellow dot as the test runs) to change to the green check mark. The green check mark on a GitHub repository indicates that the commit or pull request has been tested and meets the defined criteria for correctness.\
          If there's a red X instead, it means the tests have failed. You'll need to investigate and fix the issues in VScode, then perform the `git add`, `git commit`, and `git push` steps again.")
